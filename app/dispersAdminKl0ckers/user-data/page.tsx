...
const defaultProfilePicture = "/defaultProfile.png"; // Default image URL
        ...
        <tbody>
          {data.map((entry, index) => (
            <tr key={entry._id} className={`border-b ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
              ...
              <td className="border px-4 py-2">{getStatusBadge(entry.status)}</td>
      ...
      {selectedEntry && (
        <div className="fixed inset-0 bg-gray-700/30 bg-opacity-50 flex justify-center items-center z-99">
          <div className="bg-white p-8 rounded-md w-1/3">
            ...
            <div className="mb-4">
              <strong>Name:</strong> {selectedEntry.firstName} {selectedEntry.lastName}
            </div>
            <div className="mb-4">
