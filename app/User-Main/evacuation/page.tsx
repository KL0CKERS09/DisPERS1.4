...
export default function EvacuationAreas() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      console.log("Fetched evacuation data:", data); // 👈 Add this
      setEvacuations(data);
    };
  ...
  const filteredEvacuations = evacuations.filter(evac => {
    const matchesSearch = evac.name?.toLowerCase().includes(search.toLowerCase());
    const matchesStatus = filterStatus === 'All' || evac.status === filterStatus;    
            ...
            <tbody>
              {filteredEvacuations.map((evac, idx) => (
                <tr key={idx} className="border-t">
                  ...
                  <td className={`px-4 py-2 font-medium ${statusColor[evac.status as keyof typeof statusColor] || ''}`}>
                    {evac.status}
                  </td>
